...
export async function GET(request: NextRequest) {
  ...
  const themeName = searchParams.get('theme') || 'ink_dark';
  ...
  if (!username) {
    return new NextResponse(
      generateErrorCard('Username is required', getTheme('ink_dark')),
      {
        status: 400,
        headers: {
  ...
  try {
    ...
    return new NextResponse(
      generateErrorCard(errorMessage, getTheme('ink_dark')),
      {
        status: 500,
        headers: {
          'Content-Type': 'image/svg+xml',
